...
async function bundleWasm({ path: wasmPath, wrap = true, imports }) {
  ...
  // );
  imports &&= new Set(imports); // we modify imports, so we clone it first
  if (wrap) {
  ...
  for (let mod of moduleList) {
    ...
    let imports = mod.ast.body[0].fields.filter(
...
function addWrappers(ast, imports, wrapperModules) {
  ...
  let isSameExport = (m) => (n) =>
    n.type === "ModuleExport" && n.name === m.name;
...
function treeshakeModule(
  ast,
  { imports, moduleName, isEntryPoint, importMap }
) {
